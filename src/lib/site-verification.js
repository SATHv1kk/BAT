...
export function judgeProbe({ requestedUrl, finalUrl, title = '', textLen = 0, rowsFound = null }) {
  ...
  try {
    ...
    if (a.search && !b.search) {
      ...
      ok = false;
      reasons.push(`redirected to a different origin (${a.origin} → ${b.origin})`);
    } else if (a.href !== b.href) {
      reasons.push(`redirected within the site (${a.href} → ${b.href}) — not fatal, but record the final form`);
    }
  } catch {
