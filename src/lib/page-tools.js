...
(function () {
  ...
  async function mapCheckboxRefs(tabId, registry) {
    if (!registry) return [];
    // Registry entries arrive in two shapes: the accessibility-tree path stores
    // `tag` (the tree line's leading word, e.g. "checkbox"), while find-derived
    // entries store `role`. The old filter read only `e.role`, which tree
    // entries never have — so a checkbox whose label didn't literally contain
    // the word "checkbox" (e.g. "Accept terms", "I agree") was silently dropped
    // and course-mode pages never surfaced their checkbox refs to the model.
    const entries = Object.entries(registry).filter(([, e]) =>
      e.role === 'checkbox' || e.tag === 'checkbox' || /checkbox/i.test(e.label || '')
    );
