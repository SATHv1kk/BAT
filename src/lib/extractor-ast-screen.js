...
// extractor-screen.js rather than implied away. What this
...
// Objects whose identity is itself a navigation/network capability when held
// under any name. `var n = navigator; n.sendBeacon(...)` and
// `var h = history; h.back()` are indistinguishable from the direct call, so
// these get their own alias categories tracked in Pass 1.
...
// DOM mutation/construction — an extractor reads, it does not act, and element
// creation is how page text is exfiltrated via a .src/.href remote load.
const DOM_MUTATION_PROPS = new Set(['appendChild', 'append', 'insertBefore', 'insertAdjacentHTML', 'prepend', 'after', 'before', 'replaceWith']);
const DOM_MUTATION_FNS = new Set(['createElement', 'createElementNS']);
...
//
// `document.defaultView` is window — the document object's own back-reference
// to the window that owns it. `document` is NOT window identity (an extractor
// legitimately holds document), but `document.defaultView` and its aliases
// absolutely are, so that specific member access is treated as global-yielding.
// Without this, `var e = document.defaultView.eval; e('...')` reached eval
// through an untracked route.
function resolvesToGlobalIdentity(node, globalAliases, documentAliases = new Set()) {
  ...
  if (node.type === 'MemberExpression') {
    ...
    // document.defaultView → window, and its aliases.
    if (propName === 'defaultView') {
      return isIdentifier(node.object, 'document') || documentAliases.has(node.object?.name || '');
    }
    return propName != null && GLOBAL_YIELDING_PROPS.has(propName) && resolvesToGlobalIdentity(node.object, globalAliases, documentAliases);
  }
  return false;
}
...
function grantsCapability(init, globalAliases, capabilityAliases, documentAliases = new Set()) {
  ...
  if (init.type === 'MemberExpression') {
    ...
    if (prop && CAPABILITY_PROPS.has(prop) && resolvesToGlobalIdentity(init.object, globalAliases, documentAliases)) return true;
  }
...
export function screenExtractorSourceAst(src) {
  ...
  const domMutationAliases = new Set();
  const hits = new Set();
  ...
  const seed = (name, init) => {
    ...
    if (grantsCapability(init, globalAliases, capabilityAliases, documentAliases)) capabilityAliases.add(name);
    if (isIdentifier(init, 'navigator')) navigatorAliases.add(name);
    if (isIdentifier(init, 'history')) historyAliases.add(name);
    // `var c = document.createElement;` / `var a = document.body.appendChild;`
    // — a bare alias of a DOM-construction/mutation method. Tracked so a later
    // `c("img")` / `a(x)` call is caught even though the alias name is opaque.
    if (init.type === 'MemberExpression' && staticPropName(init)
        && (DOM_MUTATION_PROPS.has(staticPropName(init)) || DOM_MUTATION_FNS.has(staticPropName(init)))) {
      domMutationAliases.add(name);
    }
  };
  ...
  for (let round = 0; round < 6; round++) {
    const before = globalAliases.size + capabilityAliases.size + documentAliases.size + navigatorAliases.size + historyAliases.size + domMutationAliases.size;
    walkAncestor(ast, {
    ...
    if (globalAliases.size + capabilityAliases.size + documentAliases.size + navigatorAliases.size + historyAliases.size + domMutationAliases.size === before) break; // fixed point
  }
  ...
  walkAncestor(ast, {
    ...
    MemberExpression(node) {
      ...
      if ((prop === 'sendBeacon') && (isIdentifier(node.object, 'navigator') || navigatorAliases.has(node.object?.name || ''))) {
        flag('network access (sendBeacon)');
      }
      if (prop === 'credentials' && (isIdentifier(node.object, 'navigator') || navigatorAliases.has(node.object?.name || ''))) {
        flag('credential access (Credential Management)');
      }
      if ((prop === 'click' || prop === 'submit' || prop === 'requestSubmit')) {
      ...
      if ((prop === 'open' || prop === 'location') && resolvesToGlobalIdentity(node.object, globalAliases, documentAliases)) {
        flag('navigation (window.open / window.location), reached via an alias');
      }
      if (prop === 'location' && (isIdentifier(node.object, 'document') || documentAliases.has(node.object?.name || ''))) {
        flag('navigation (document.location), reached via an alias');
      }
      if ((prop === 'back' || prop === 'go' || prop === 'forward' || prop === 'replaceState' || prop === 'pushState')
          && (isIdentifier(node.object, 'history') || historyAliases.has(node.object?.name || ''))) {
        flag('navigation (history API), reached via an alias');
      }
    ...
    CallExpression(node) {
      ...
      // DOM-mutation / element-construction calls, whether via a tracked alias
      // or spelled out — `var c = document.createElement; c("img")` aliases the
      // same capability and must be caught like the direct call.
      if (node.callee.type === 'Identifier' && DOM_MUTATION_FNS.has(node.callee.name)) {
        flag(`DOM mutation/construction via an alias (\`${node.callee.name}\`)`);
      }
      if (node.callee.type === 'Identifier' && domMutationAliases.has(node.callee.name)) {
        flag('DOM mutation/construction via an alias');
      }
      if (node.callee.type === 'MemberExpression' && calleeProp && DOM_MUTATION_PROPS.has(calleeProp)) {
        flag(`DOM mutation/construction (${calleeProp})`);
      }
