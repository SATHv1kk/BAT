...
(function () {
  ...
  async function clickRef(tabId, frameId, localRef) {
    ...
    const results = await executeScript(target, (ref) => {
      function findEl(r) {
        ...
        // r is model-supplied (prompt-injectable); CSS.escape is in-page when
        // available, otherwise escape the selector-breaking characters.
        const q = (typeof CSS !== 'undefined' && CSS.escape ? CSS.escape(r) : String(r).replace(/["\\]/g, '\\$&'));
        return document.querySelector('[data-ext-ref="' + q + '"]');
      }
  ...
  async function runJavaScript(tabId, code, frameId) {
    // Screen AT THE SINK, not only at the caller. This runs in the page's MAIN
    // world with the page's origin — the single most privileged sink in the
    // extension — and the safety screen used to be applied only by app.js's
    // `javascript` action. Any future call site that forgot to screen would
    // have gotten raw MAIN-world eval with no guard at all. The screen here is
    // the last line: if it rejects, nothing executes.
    const screened = screenExtractorSource(code);
    if (!screened.ok) {
      return { success: false, error: 'run_javascript blocked by safety screen: ' + screened.reason };
    }
  ...
  async function typeIntoRef(tabId, frameId, localRef, text, clickFirst) {
    ...
    const results = await executeScript(target, (ref, value, doClick) => {
      ...
      if (!el) el = document.querySelector('[data-ext-ref="' + (typeof CSS !== 'undefined' && CSS.escape ? CSS.escape(ref) : String(ref).replace(/["\\]/g, '\\$&')) + '"]');
      if (!el) return { success: false };
