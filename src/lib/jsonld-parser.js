...
// Walks a JSON-LD node. "@graph" arrays are flattened, and every nested
// object that carries its own @type (offers, seller, brand, aggregateRating,
// review, ...) is followed recursively and added to `into` — otherwise a
// Product with `offers: {"@type":"Offer","price":"9.99"}` nested inline (the
// overwhelmingly common single-script shape) would never have its Offer
// discovered, and price/currency/availability/seller would all be missing.
function walkNode(node, into = []) {
  ...
  for (const item of items) {
    ...
    into.push(item);
    // Descend into every property whose value is an object (or array of
    // objects) that itself carries an @type — these are the nested typed
    // nodes (Offer, Organization, AggregateRating, Review, Brand) that sit
    // inside their parent instead of alongside it.
    for (const key of Object.keys(item)) {
      if (key === '@graph' || key === '@id') continue;
      const v = item[key];
      if (Array.isArray(v)) {
        if (v.some((x) => x && typeof x === 'object' && x['@type'])) walkNode(v, into);
...
export function parseJsonLdFromHtml(input) {
  ...
  if (product) {
    ...
    // extractByType already resolved nested objects (a Brand node resolves to
    // its name), so brand/image are strings here. The old object/array branches
    // were dead.
