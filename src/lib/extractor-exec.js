...
async function viaScripting(tabId, source) {
  // The scripting fast-path had NO timeout: an extractor containing
  // `while(true){}` (nothing in either screen bans loops) injected a function
  // that ran forever, hanging the panel/runner with no error ever surfacing.
  // Race it like every other executeScript in the project — the isolated world
  // of that frame still busy-loops, but the agent gets its answer and moves on.
  let timer;
  try {
    const [res] = await Promise.race([
      ...
      new Promise((_, rej) => {
        timer = setTimeout(() => rej(new Error('extractor execution timed out after 10s')), EXECUTE_TIMEOUT_MS);
      })
    ]);
