...
(function () {
  ...
  function isSensitiveField(type, autocomplete, name, id) {
    if (type === 'password' || type === 'hidden') return true;
    for (var i = 0; i < SENSITIVE_AUTOCOMPLETE.length; i++) {
      if (autocomplete.indexOf(SENSITIVE_AUTOCOMPLETE[i]) !== -1) return true;
    }
    // A `name`/`id` that names the secret is as damning as an autocomplete
    // token — a <input type="text" name="cc-number"> holds a card number even
    // though its type/autocomplete are both innocuous, and the model would
    // otherwise receive the live value as the element's name.
    var hay = ((name || '') + ' ' + (id || '')).toLowerCase();
    return /(?:cc|card|cardnumber|cvv|cvc|expiry|exp-month|exp_year|otp|one.?time|passwd|passphrase|secret|token|pin)\b/.test(hay);
  }
  ...
  function labelForText(el) {
    if (!el.id) return '';
    // A <label for="..."> points at the element BY its id, so the lookup is
    // querySelector — but the id must be CSS.escape'd before interpolation.
    // An id containing a quote, backslash, or bracket is a valid HTML id and
    // would throw a SyntaxError in a raw `label[for="..."]` string — which,
    // caught by the outer try in the generator, failed the ENTIRE page read
    // over one hostile id. CSS.escape cannot throw and keeps the selector
    // matching exactly the id that was asked for.
    var labelEl = document.querySelector('label[for="' + (typeof CSS !== 'undefined' && CSS.escape ? CSS.escape(el.id) : el.id.replace(/["\\]/g, '\\$&')) + '"]');
    if (!labelEl) return '';
  ...
  function getAccessibleName(el) {
    ...
    if (tag === 'input') {
      ...
      // A file picker's value is the local path of the chosen file
      // ("C:\fakepath\Resume.pdf") — a private detail the model has no use
      // for. Never surface it.
      if (type === 'file') return el.files?.length ? '[file selected]' : '';
    ...
    try {
      ...
      function walk(el, level) {
        ...
        if (included) {
          ...
          line += ' [' + ref + ']';
          // State markers FIRST, so a model scanning a long tree can see at a
          // glance what is on/off/disabled without reading the whole line.
          // The `(checked)` / `(selected)` forms are a parse contract — see
          // countAnsweredInTree in the panel — so they are kept verbatim.
          ...
          if ((tag === 'input' && (inputType === 'checkbox' || inputType === 'radio')) || explicitRole === 'checkbox' || explicitRole === 'radio') {
      ...
      // Orientation header: one compact line that tells the model what kind of
      // page this is before it reads a single tree line. "What am I looking
      // at?" is the first question a good observer answers, and a tree dump
      // without a heading forces the model to infer it from indentation.
      if (!refId) {
        var interactive = 0;
        for (var ri = 0; ri < lines.length; ri++) {
          if (/\[ref_\d+\]/.test(lines[ri])) interactive++;
        }
        pageContent = 'PAGE: ' + (document.title || '(untitled)')
          + ' | ' + count + ' element(s), ' + interactive + ' interactive'
