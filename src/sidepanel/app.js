...
// ── State ─────────────────────────────────────────────────────────
// The model is NOT state — it is the constant MODEL. It used to be mutable
// panel state backed by chrome.storage, which is what allowed an install to
// end up pinned to an id the API had stopped serving.
let apiKey          = '';
...
// Index into THINKING_STAGES. Advances one rung per reasoning-related 400 and
// never resets within a session. The old flag was a boolean, which could only
// express "send the fields" or "omit them" — and omitting them does NOT turn
// thinking off, so a provider that rejected one field left the panel 400ing
// forever. The ladder's last two rungs are recoveries the boolean could not
// reach: state it disabled, then stop mentioning it at all.
let thinkingStage = 0;
let currentGoal       = '';
let debugLog          = [];
...
let allowAllSites   = false; // replaced at init by storage; a fresh install defaults to true
...
// Item 6 — the model's working checklist. The model creates it with the
// set_checklist tool and updates it as it works; it is injected into every
// subsequent system message so the model always knows where it is in the goal.
// Persisted with the session so a restored conversation keeps its checklist.
let checklist       = [];    // string lines, "[x] done" / "[ ] open"
let runEventLog     = [];    // "what happened" log for the current run (goal + actions)
// Whether the current run has produced a checklist via set_checklist. A stale
// checklist from a PREVIOUS goal must not be injected into a new goal — the
// model should create a fresh one (the step-1 nudge fires while this is false).
let checklistCreatedThisRun = false;
...
// Which rung of the ladder this request rides on. MODEL supports thinking, so
// this starts at full effort and degrades only as the provider actually rejects
// something — there is no per-model capability check left to make.
function currentThinkingMode() {
  return THINKING_STAGES[Math.min(thinkingStage, THINKING_STAGES.length - 1)];
}

// One-time cleanup of the model the picker used to persist. Left behind it is
// merely dead, but it is also misleading: someone debugging a wrong-model report
// would find a stale id in storage that nothing reads any more.
function discardStoredModel() {
  chrome.storage.local.remove('bat_model')
    .catch((e) => debugEntry('discard_stored_model_failed', { error: e?.message }));
}
...
async function executeAgentAction(tabId, action) {
  ...
  switch (action.action) {
    ...
    case 'click_coords': {
      // Refuse at the point of action, not just in the schema. Coordinates are
      // only meaningful if they came from a screenshot; with vision off the tool
      // is withheld from the tool list, but the legacy JSON protocol and a
      // restored session can both still name it, and a blind click at a guessed
      // pixel on an approved site is the one failure here worth being paranoid
      // about — it is unverifiable after the fact.
      if (!visionEnabled) {
        return { success: false, error: 'click_coords is unavailable: this model has no image input, so viewport coordinates cannot have been observed. Locate the element with read_page or find, then use left_click with its ref.' };
      }
...
// What one message actually costs on the wire. reasoning_content is sent on
// every subsequent tool-calling request, so a budget that counted only
// `content` under-measured every assistant turn — and thinking blocks are not
// small. Undercounting here is what let the real context outgrow the budget
// while the budget believed it had room.
function messageWeight(m) {
  return messageContentLength(m.content) + (m.reasoning_content?.length || 0);
}
...
function pruneSessionForBudget() {
  ...
  let total = session.reduce((n, m) => n + messageWeight(m), 0);
  if (total <= CONTEXT_CHAR_BUDGET) return;

  const trimEnd = session.length - KEEP_RECENT_MESSAGES;

  // Pass 1 — trim oldest-first, as before. Only `content` is ever shortened:
  // reasoning_content must be replayed FULLY or the request 400s, so a thinking
  // block is all-or-nothing and can only leave with its message in pass 2.
...
function dropOldestMessages(total) {
  ...
  while (cut < keepFrom && total - droppedChars > COMPACT_TARGET_CHARS) {
    ...
    for (let i = cut; i < cut + span; i++) {
      droppedChars += messageWeight(session[i]);
      droppedCount++;
    }
    cut += span;
  }
...
function repairSessionToolCalls() {
  ...
  for (let i = 0; i < session.length; i++) {
    const m = session[i];
    // A `tool` message at the very start of the session is an orphan: its
    // assistant tool_call was cut off by the persistence window (the local
    // mirror's slice boundary can land inside a tool-result span). Forwarding
    // it verbatim makes the API 400 on the first step, and the error then
    // mis-triggers the native-tools fallback. Drop it — the conversation still
    // reads correctly without a result that has no question.
    if (m.role === 'tool' && fixed.length === 0) continue;
    fixed.push(m);
    if (m.role === 'assistant' && m.tool_calls?.length) {
...
// Keeps only what the wire format defines. reasoning_content stays: DeepSeek
// rejects a tool-calling request whose assistant turns dropped it, so this is
// the message that has to remember it — verbatim, never trimmed.
...
async function buildSnapshot(state, { force = false, skipAutoScroll = false } = {}) {
  ...
  if (!pageData) {
    ...
    // If the failure follows a direct URL navigation, count it toward the
    // search-first recovery threshold — and clear the marker immediately so a
    // SINGLE failed URL attempt counts once. Otherwise clicks/waits/scrolls
    // over a persistently dead page (which are not URL attempts) keep
    // inflating the counter and the escalation blames a stale URL.
    if (state.lastNavUrl) {
      state.navFailures = (state.navFailures || 0) + 1;
      state.lastNavUrl = null;
    }
    ...
    msg += '\n';
    // Search-first escalation: after repeated direct-URL failures, STOP
    // navigating by URL entirely and drive the site's own search bar with plain
    // words. Sites that block URL loads almost always serve their own search
    // UI, and the words-to-pattern path is exactly what a human uses.
    if (state.navFailures >= 3) {
      msg += `escalation: direct URL navigation has failed ${state.navFailures}× in a row`
        + (state.lastNavUrl ? ` (last URL: ${state.lastNavUrl})` : '') + '. '
        + 'STOP navigating by URL. Go back to the site home/search page and use its '
        + 'search bar with PLAIN WORDS instead (find the search box, type the query, press Enter). '
        + 'Do not attempt the URL again. If there is no search UI, tell the user the site is blocking '
        + 'and move on.\n';
    } else if (state.unreadableStreak >= 3) {
  ...
  // A clean read means the URL worked — reset the direct-URL failure counter.
  if (state.navFailures) {
    state.navFailures = 0;
  }
  state.lastNavUrl = null;
  ...
  if (!force && fp && fp === state.lastFingerprint) {
    ...
    out += `- note: accessibility tree UNCHANGED since previous observation (${state.unchangedStreak}× in a row) — your last action may not have taken effect.\n`;
    ...
    const resumeHint = buildResumeDialogHint(pageData, state.tabId, state.resumeClickStreak);
    const quizHint = buildQuizPhaseHint(pageData);
    if (resumeHint) out += resumeHint.trimEnd() + '\n';
    if (quizHint) out += quizHint.trimEnd() + '\n';
...
async function probeTemplateUrl(state, url) {
  const BT = window.BrowserTools;
  const previous = (await chrome.tabs.get(state.tabId).catch(() => null))?.url;
  try {
    ...
    const probe = res?.result || { finalUrl: '', title: '', textLen: 0 };
    // The probe is a THROWAWAY navigation on the working tab — the model is
    // only asked to look at it long enough to judge the template. Leaving the
    // tab on the probed URL presented it to the next snapshot as if it were the
    // user's target, so the model started acting on a page it was only meant to
    // inspect. Restore what was there before.
    if (previous && previous !== url && /^https?:/.test(previous)) {
      await BT.navigate(state.tabId, previous).catch(() => {});
    }
    return probe;
  } catch (e) {
...
async function runTool(state, name, args, step, opts = {}) {
  ...
  if (name === 'open_tab') {
    ...
    // Optional chaining on the METHOD then `.catch` on the result: if the
    // method itself is missing, `?.` yields undefined and `.catch` throws
    // TypeError. Guard the whole call.
    if (window.BrowserTools?.waitForPageIdle) await window.BrowserTools.waitForPageIdle(t.id, 15000).catch(() => {});
  ...
  if (name === 'save_file' || name === 'read_file' || name === 'list_files' || name === 'give_file') {
    try {
      if (name === 'save_file') {
      ...
      if (name === 'give_file') {
        const content = await Workspace.readFile(args.filename);
        const text = content === '(empty file)' ? '' : content;
        renderFileAttachment(args.filename, text);
        return `File shown in the chat: ${args.filename} (${text.length} chars). The user can download it from the attachment in the conversation.`;
      }
      ...
      // Prefixing a missing file with "Workspace error" made a typo look like
      // infrastructure trouble — the reaction was to go chase a folder
      // reconnect that had nothing to do with it. Label each cause as itself.
      if (e.fileNotFound) return `File not found: ${e.message}`;
      if (e.workspaceDisconnected) return `Workspace not writable: ${e.message}`;
  ...
  // Track direct-URL navigation failures so the loop can steer the model to the
  // site's own search UI instead of reloading a blocked URL a fourth time.
  // A navigation "succeeds" at the transport level (the tab navigated) — the
  // failure shows up as an unreadable page on the FOLLOWING read. So we record
  // the navigate here, and buildSnapshot (below) inspects it when the read
  // fails. The count is reset by a clean read in buildSnapshot.
  if (name === 'navigate' && action.url) {
    state.lastNavUrl = action.url;
  }
...
async function switchAgentTab(state, newTabId, note) {
  if (state.tabId != null && state.tabId !== newTabId) {
    ...
    if (window.BrowserTools?.releaseDebugger) await window.BrowserTools.releaseDebugger(state.tabId).catch(() => {});
    if (state.borderShown) await showAgentBorder(state.tabId, false).catch(() => {});
  }
  state.tabId = newTabId;
  state.lastFingerprint = '';
  state.borderShown = false;
  // A tab switch is a fresh context: nav-failure state for the previous tab's
  // URL must not leak into this one (a protected/mid-load new tab would
  // otherwise inflate a counter meant for direct-URL navigation failures).
  state.navFailures = 0;
  state.lastNavUrl = null;
...
async function withAgentTab(state, fn, { allowDeadTab = false } = {}) {
  ...
  if (state.pendingChildTabId != null) {
    ...
    if (child) {
      ...
      if (window.BrowserTools?.waitForPageIdle) await window.BrowserTools.waitForPageIdle(childId, 8000).catch(() => {});
    }
  }
...
async function runAgentLoop(userText) {
  ...
  const state = {
    ...
    // Direct-URL navigation failures. Sites increasingly block or error on raw
    // URL loads that "look automated", so the recovery is to reach the same
    // content THROUGH the site's own search UI instead of reloading the URL a
    // fourth time. Resets when a page actually reads cleanly.
    navFailures: 0,
    lastNavUrl: null,
    // How many times the "model said no before trying" nudge has fired. Caps the
    // pushback so a stubborn model cannot loop forever at step 1/2.
    refusalNudges: 0,
    // How many tool calls have actually RUN this run. The refusal nudge must
    // not claim "you have not attempted any action" after the model already
    // read the page — it should only fire when genuinely nothing has run.
    toolsAttempted: 0
  };
  ...
  runEventLog = [`goal: ${userText.slice(0, 300)}`];
  checklistCreatedThisRun = false;
  ...
  try {
    while (step < MAX_STEPS && !stopRequested) {
      ...
      const systemParts = [buildSystemPrompt({ visionEnabled, nativeToolsEnabled })];
      // Item 6 — the working checklist and the run event log are injected every
      // turn so the model always knows where it is in the goal and what it has
      // already done. This is what makes long multi-step work reliable: the
      // model is never working from memory.
      if (checklist.length && checklistCreatedThisRun) {
        // Cap the injected size hard: the checklist is model-controlled and a
        // noisy model could inflate it unbounded, and it is rebuilt fresh every
        // step so pruneSessionForBudget's conversation budget never sees it.
        const injectedChecklist = checklist.length > 40 ? checklist.slice(-40) : checklist;
        systemParts.push('YOUR CURRENT CHECKLIST (keep this up to date with set_checklist; tick items [x] as you finish them):\n' + injectedChecklist.join('\n'));
      } else if (step === 1) {
        systemParts.push('After your FIRST page read, call set_checklist to create a checklist for this goal — break it into small ordered steps. Update it with set_checklist whenever a step completes.');
      }
      if (runEventLog.length) {
        systemParts.push('WHAT HAS HAPPENED SO FAR THIS RUN (most recent last):\n' + runEventLog.slice(-15).join('\n'));
      }
      const messages = [{
        role: 'system',
      ...
      // and for recovery (stuck page / failed action); cheap for routine steps.
      // "low" not "medium" — medium is not in the enum, it is only tolerated as
      // an alias the API rewrites to high, so the routine-step path was quietly
      // paying for high effort on every step it meant to economize on.
      const effort = (step === 1 || state.unchangedStreak > 0 || state.unreadableStreak > 0 || state.lastActionFailed) ? 'high' : 'low';

      let result;
      const liveBubble = addStreamingBubble();
      try {
        result = await callDeepSeekAPI(apiCtx, getActiveKey(), MODEL, messages, {
          max_tokens: 8192,
          step,
        ...
        // Model/endpoint doesn't accept the tools param → fall back to JSON
        // protocol. Checked BEFORE the reasoning ladder: a gateway that rejects
        // `tools` typically says "Unknown parameter: tools", which the reasoning
        // regex would otherwise match first — burning the whole 3-rung thinking
        // ladder (three full-prompt retries + three misleading breadcrumbs)
        // before the tools fallback ever got a chance.
        if (nativeToolsEnabled && err.apiStatus === 400
            && /tool|function/.test(err.message || '')) {
        ...
        // A rejected reasoning field, or a complaint about reasoning_content
        // itself, drops us one rung down THINKING_STAGES and retries. The last
        // rung ("omit") cannot provoke these errors, so this can no longer spin:
        // once exhausted the error falls through to real reporting below.
        if (thinkingStage < THINKING_STAGES.length - 1 && err.apiStatus === 400
            && /reasoning_content|reasoning_effort|thinking|unknown (?:field|parameter|argument)|unrecognized|unexpected key/i.test(err.message || '')) {
          const from = currentThinkingMode();
          thinkingStage++;
          const to = currentThinkingMode();
          debugEntry('thinking_stage_fallback', { step, from, to, reason: err.message });
          addActivity(step, 'warn', 'Model rejected the reasoning parameters',
            `Retrying with thinking "${to}" instead of "${from}" — the model still works.`, 'warn');
          step--;
          continue;
        }
        ...
        if (err.apiStatus === 400 || err.apiStatus === 404) {
          if (/model/i.test(err.message || '')) {
            // There is no picker to redirect the user to any more, so the only
            // honest instruction is the one that actually fixes it: the account
            // does not serve this id, or the id has been retired. Name the
            // constant, since editing it is the entire remedy.
            addActivity(step, 'error', `Model "${MODEL}" was rejected by the API`,
              `${err.message}\n\nYour account may not have access to this model, or the id has been retired. `
              + 'BAT uses exactly one model: set MODEL in src/shared/constants.js to an id your account serves '
              + '(see https://api-docs.deepseek.com/quick_start/pricing) and reload the extension.', 'error');
            break;
          }
        }
      ...
      if (result.content) {
        if (toolCalls.length && toolCalls[0].legacy) {
          ...
          showLiveStatus('finishing');
          addMessage('assistant', result.content);
        }
      }

      // Item 11 — a model that "says no" without trying. If the model replies
      // with a plain-text refusal/hedge ("I can't", "I don't have access",
      // "unable to", "not possible") and has not yet made ANY tool call in this
      // run, the goal is almost certainly still achievable — the model gave up
      // pre-emptively. Nudge it to actually try once before accepting defeat.
      // This only fires on the FIRST step(s) of a run; a later plain reply is a
      // real answer and is respected.
      if (!toolCalls.length && result.content && step <= 2 && state.refusalNudges < 2 && !state.toolsAttempted) {
        const text = result.content.toLowerCase();
        const refusalLike = /i (?:can'?t|cannot|can not|don'?t|do not|won'?t|will not)\b|unable to|not (?:able|possible|allowed)|no (?:access|permission)|i have no (?:way|ability)|this is (?:not|beyond)|i (?:am )?unable|can'?t (?:find|access|do|get|open)/i;
        if (refusalLike.test(text) && !state.lastActionFailed) {
          state.refusalNudges++;
          let nudge = `You have not actually attempted any action yet. Do not give up — use the tools. `;
          if (state.tabId != null) {
            nudge += 'Read the page first (read_page), locate the relevant controls, and try. '
              + 'If a URL navigation is blocked, use the site search bar with words. '
              + 'Only report an obstacle to the user after you have genuinely tried and failed.';
          } else {
            nudge += 'Open a tab or navigate to the target, then work through the steps.';
          }
          addActivity(step, 'warn', 'Model declined before trying', nudge, 'warn');
      ...
      for (let ti = 0; ti < toolCalls.length; ti++) {
        ...
        // Item 6 — the model maintains a working checklist via this tool. It is
        // stored here, injected into every system message, and persisted.
        if (name === 'set_checklist') {
          const items = Array.isArray(args.items) ? args.items.map(i => String(i).slice(0, 200)) : [];
          // Hard cap on stored items too — not just the injected slice — so a
          // runaway checklist can't grow the persisted session unbounded.
          checklist = items.slice(0, 60);
          checklistCreatedThisRun = true;
          persistSession();
          const doneCount = checklist.filter(i => /^\[x\]/i.test(i)).length;
          pushToolResult(tc, `Checklist updated — ${checklist.length} item(s), ${doneCount} done. It will appear in your next context.`);
          addActivity(step, 'result', 'Checklist updated', checklist.join('\n'), 'success');
          continue;
        }

        addActivity(step, 'act', `Tool: ${name}`, summarizeArgs(args), '', true);
        state.toolsAttempted++;
        ...
        const output = await withAgentTab(state, () => runTool(state, name, args, step, { skipSnapshot }), {
          allowDeadTab: ['list_tabs', 'open_tab', 'switch_tab', 'save_file', 'read_file', 'list_files',
            'append_rows', 'collect_rows', 'export_rows', 'data_report', 'run_control', 'ats_fetch', 'record_not_found',
            'set_checklist', 'give_file'].includes(name)
        });
        pushToolResult(tc, output);
        // Item 6 — a compact "what happened" record, trimmed so it stays cheap.
        const trimmedOut = String(output).replace(/\s+/g, ' ').slice(0, 180);
        runEventLog.push(`step ${step} ${name}: ${trimmedOut}`);
        if (runEventLog.length > 60) runEventLog.shift();
      }
      if (doneCalled) break;
...
//   • chrome.storage.session — the fast primary. Survives side-panel close but
//     Chrome clears it on browser exit, so a browser restart used to lose the
//     whole conversation with nothing anywhere to bring it back.
//   • chrome.storage.local — survives browser shutdown too. It gets a BOUNDED
//     copy of the same state (trimmed messages, truncated detail, latest
//     goal/usage/debug) so a restart restores the conversation instead of
//     starting from zero. The two are written together; restore prefers session
//     and falls back to local.
//
// Note: reasoning_content must be replayed verbatim on tool-calling turns (the
// wire contract), so the LOCAL copy must NOT trim it from assistant messages —
// trimming only touches user text and detail fields. An under-replay from a
// too-small local copy would 400 on the first step after restart, which is
// worse than the truncation it would avoid.
...
function flushSessionNow() {
  ...
  const payload = {
    ...
    bat_checklist: checklist,
    bat_eventlog: runEventLog.slice(-60),
  ...
  chrome.storage.session.set(payload).catch((e) => {
    // This used to be `.catch(() => {})`. A failure here means the conversation
    // silently stops surviving panel close — the user loses a long run's history
    // with nothing anywhere to explain it, which is exactly the failure mode
    // "Errors get breadcrumbs" exists to prevent. Sessions also got larger when
    // reasoning_content became part of the persisted message (the API requires it
    // replayed), so the quota is closer than it was.
    debugEntry('persist_session_failed', {
      error: e?.message || String(e),
      messages: session.length,
      approxChars: session.reduce((n, m) => n + messageWeight(m), 0)
    });
  });
  // Mirror to local so the conversation survives browser shutdown. Bounded and
  // detail-trimmed to stay inside the local quota; reasoning_content preserved
  // per the note above.
  const localCopy = {
    bat_session: session.slice(-80),
    bat_goal: currentGoal,
    bat_checklist: checklist.slice(-50),
    bat_eventlog: runEventLog.slice(-60),
    bat_ui: uiLog.slice(-300).map(e =>
      e.detail && typeof e.detail === 'string' && e.detail.length > 2000
        ? { ...e, detail: e.detail.slice(0, 2000) } : e),
    bat_usage: usageTotals,
    bat_debug: debugLog.slice(-100).map(e =>
      typeof e.detail === 'string' && e.detail.length > 300 ? { ...e, detail: e.detail.slice(0, 300) } : e),
    bat_saved_at: Date.now()
  };
  chrome.storage.local.set(localCopy).catch((e) => {
    // Local persistence is the browser-restart survival path; a quota failure
    // there means a restart loses history, which is worth a breadcrumb.
    debugEntry('persist_local_failed', {
      error: e?.message || String(e),
      messages: localCopy.bat_session.length
    });
  });
}
...
// A session persisted by a build that discarded reasoning_content restores with
// assistant tool-call turns that can never satisfy the replay requirement — the
// thinking blocks are simply gone, and no amount of correct code downstream can
// invent them. Left alone, such a session 400s on its first step forever, so the
// user's only recovery would be to notice and clear the chat by hand.
//
// Thinking mode is what makes the replay mandatory, so the repair is to run this
// restored conversation with thinking off. History is kept, the run continues,
// and the cost is explicit rather than a silent dead panel. A fresh conversation
// is unaffected and starts at full effort.
function reconcileRestoredReasoning() {
  const orphaned = session.some((m) => m.role === 'assistant' && m.tool_calls?.length && !m.reasoning_content);
  if (!orphaned) return;
  thinkingStage = THINKING_STAGES.indexOf('disabled');
  debugEntry('restored_without_reasoning', { messages: session.length, thinking: 'disabled' });
}

async function restoreSession() {
  try {
    // Prefer the fast session store; fall back to the local mirror (which
    // survives browser shutdown) when Chrome cleared session storage on exit.
    const s = await chrome.storage.session.get(['bat_session', 'bat_goal', 'bat_ui', 'bat_usage', 'bat_debug', 'bat_checklist', 'bat_eventlog']);
    const sessionStore = s.bat_session;
    let restored = sessionStore;
    let goal = s.bat_goal;
    let ui = s.bat_ui;
    let usage = s.bat_usage;
    let dbg = s.bat_debug;
    let chk = s.bat_checklist;
    let evt = s.bat_eventlog;
    if (!Array.isArray(sessionStore) || !sessionStore.length) {
      const l = await chrome.storage.local.get(['bat_session', 'bat_goal', 'bat_ui', 'bat_usage', 'bat_debug', 'bat_checklist', 'bat_eventlog']);
      if (Array.isArray(l.bat_session) && l.bat_session.length) {
        restored = l.bat_session;
        goal = l.bat_goal;
        ui = l.bat_ui;
        usage = l.bat_usage;
        dbg = l.bat_debug;
    ...
    session = restored;
    currentGoal = goal || '';
    reconcileRestoredReasoning();
    if (Array.isArray(chk)) checklist = chk;
    if (Array.isArray(evt)) runEventLog = evt;
    ...
    if (usage) usageTotals = { prompt: 0, completion: 0, requests: 0, billable: 0, ...usage };
    if (Array.isArray(dbg)) debugLog = dbg;
    restoringUi = true;
    for (const e of ui || []) {
      if (e.kind === 'message') addMessage(e.role, e.text);
      else if (e.kind === 'activity') addActivity(e.step, e.phase, e.title, e.detail, e.tone);
      else if (e.kind === 'file') renderFileAttachment(e.filename, e.detail || '');
    }
    restoringUi = false;
    uiLog = Array.isArray(ui) ? ui : [];
    updateDebugStatus();
    return true;
  } catch {
    restoringUi = false;
...
// ── Chat scrolling ────────────────────────────────────────────────
// Every append used to do `chatEl.scrollTop = chatEl.scrollHeight`
// unconditionally. During a run that fires several times a second, so scrolling
// up to read an extractor's source or an earlier error was impossible — the view
// snapped back to the bottom before you could read a line, and the faster the
// agent worked the more firmly you were pinned there.
//
// Stick to the bottom only while the reader is ALREADY there. The moment they
// scroll away, stop yanking them back and surface a button that says how far
// behind they are.
//
// The threshold is deliberately TINY. It used to be 80px: content appends fast
// during a run, and while the user was still within 80px of the bottom the
// next append kept pulling them back down — so they could never actually get
// any distance up, which read exactly as "the chat won't let me scroll up". A
// few pixels is enough to catch an accidental wheel tick, but any deliberate
// scroll-up releases the pin immediately.
const STICK_THRESHOLD_PX = 8;
let stickToBottom = true;

function isNearBottom() {
  return chatEl.scrollHeight - chatEl.scrollTop - chatEl.clientHeight <= STICK_THRESHOLD_PX;
}
...
function updateJumpButton() {
  ...
  jumpLatestBtn.hidden = stickToBottom;
  if (!stickToBottom) {
    jumpLatestBtn.textContent = `↓ Jump to latest`;
  }
}

// Called after appending anything to the chat.
function keepPinnedToBottom() {
  if (stickToBottom) chatEl.scrollTop = chatEl.scrollHeight;
  else updateJumpButton();
}

// Scroll up even a little → release the pin. A tiny debounce lets programmatic
// scrolls (from scrollChatToBottom) settle before re-evaluating.
let scrollEvalTimer = null;
chatEl.addEventListener('scroll', () => {
  clearTimeout(scrollEvalTimer);
  scrollEvalTimer = setTimeout(() => {
...
if (jumpLatestBtn) jumpLatestBtn.addEventListener('click', scrollChatToBottom);
...
function addMessage(role, text, { persist = true } = {}) {
  ...
  // A message the USER just sent always wins the scroll — they acted, so they
  // mean to be looking at the bottom.
  if (role === 'user') scrollChatToBottom();
  else keepPinnedToBottom();
...
// Turn-level status driver. The old version appended a chat bubble and streamed
// the model's raw output into it token-by-token — exactly the "full context in
// the middle" noise this redesign removes. The chat now only shows final
// answers; during generation this just flips the minimal status line between
// Thinking and Working, and `remove()` is a no-op that leaves no chat artifact.
function addStreamingBubble() {
  showLiveStatus('thinking');
  return {
    update(text, kind) {
      // reasoning_content is display-only silence (see deepseek.js): thinking
      // is captured and replayed on the wire, but never rendered. Content is
      // what "Working…" means.
      if (kind === 'content' && text) showLiveStatus('working');
      else showLiveStatus('thinking');
    },
    remove() {}
  };
}
...
// tool call. The phases are deliberately few and minimal — "Thinking…" while
// the model is generating, "Working…" while a tool is being executed, and a
// brief "Finishing…" when the turn is wrapping up. Full detail still reaches
// the debug log (Copy debug log is unaffected); the chat only ever shows final
// answers and genuine milestones now.
const STATUS_PHASES = {
...
function showLiveStatus(phase, extra = '') {
  if (!liveStatusEl || !liveStatusTextEl) return;
  const p = STATUS_PHASES[phase] || { text: phase || '' };
  liveStatusTextEl.textContent = extra ? `${p.text} — ${String(extra).slice(0, 80)}` : p.text;
  liveStatusEl.classList.toggle('error', phase === 'error');
  liveStatusEl.classList.toggle('done', phase === 'done');
...
function addActivity(step, phase, title, detail, tone = '', live = false, persist = true) {
  ...
  if (live) {
    ...
    //
    // Route by tone/phase to the minimal status word: executing tools shows
    // just "Working…", a failure shows the error word with the small note, a
    // success shows "Done". No detail paragraph is ever rendered here.
    if (phase === 'error' || tone === 'error') {
      showLiveStatus('error', title || '');
    } else if (tone === 'success' || phase === 'done') {
      showLiveStatus('done', title || '');
    } else if (phase === 'act') {
      showLiveStatus('working');
    } else {
      showLiveStatus('working', title || meta.title);
    }
    return null;
  }
  if (persist && !restoringUi) {
  ...
  if (detail) {
    // Errors render as a SMALL note, never a paragraph. The full detail already
    // reached the debug log at the top of this function; the chat should tell
    // the user something failed in one glanceable line, not dump the payload.
    const isError = tone === 'error' || phase === 'error';
    const display = isError
      ? String(detail).replace(/\s+/g, ' ').trim().slice(0, 160)
      : detail;
    const detailEl = document.createElement('div');
    detailEl.className = 'activity-detail' + (display.length > 280 ? ' collapsed' : '');
    detailEl.textContent = display;
    div.appendChild(detailEl);

    if (!isError && display.length > 280) {
...
// Render a workspace file as a downloadable attachment card in the chat. The
// user has no way to browse the workspace folder (embedded storage or a pinned
// folder), so when they ask to "see" or "download" a file, this is the surface
// that gives it to them: a filename line and a Download button that creates a
// real file via a Blob URL. No content preview — the file itself is what they
// asked for, and dumping TSV/CSV bodies into the chat would be unusable.
function renderFileAttachment(filename, content) {
  const div = document.createElement('div');
  div.className = 'message activity file-attachment';

  const top = document.createElement('div');
  top.className = 'activity-top';

  const nameEl = document.createElement('span');
  nameEl.className = 'activity-title';
  nameEl.textContent = `📄 ${filename} (${(content || '').length} chars)`;
  top.appendChild(nameEl);

  const downloadBtn = document.createElement('button');
  downloadBtn.className = 'btn-ghost file-download';
  ...
  downloadBtn.onclick = () => {
    try {
      ...
      addActivity(null, 'warn', 'Download failed', e?.message || String(e), 'warn');
    }
  };
  ...
  chatEl.appendChild(div);
  keepPinnedToBottom();
  // Record in uiLog so the attachment survives a panel reopen/restart — the
  // card used to vanish from the transcript (only addMessage/addActivity
  // persisted), leaving "the file was shown" with no card present.
  if (!restoringUi) {
    uiLog.push({ kind: 'file', filename, detail: (content || '').slice(0, 500), size: (content || '').length });
    persistSession();
  }
  return div;
}
...
function clearChat() {
  ...
  // This button wipes EVERYTHING — chat, collected data (collections,
  // extractors, runs), and workspace files — not just the conversation. That
  // used to happen silently, so a user wanting to reset a chat lost the entire
  // collected dataset with no warning. Confirm before any destructive step.
  if (typeof window !== 'undefined' && window.confirm) {
    const ok = window.confirm(
      'Clear EVERYTHING?\n\nThis removes the conversation AND all collected data, extractors, and workspace files. '
      + 'This cannot be undone.\n\nUse "Stop" if you only want to end the current task.'
    );
    if (!ok) return;
  }
  session = [];
  ...
  debugLog = [];
  currentRunId = null;
  // A fresh conversation earns a fresh ladder. Without this, a session restored
  // from an older build (which forces thinking off — see
  // reconcileRestoredReasoning) would keep every later chat in this panel
  // degraded, long after the history that required it was cleared.
  thinkingStage = 0;
  uiLog = [];
  usageTotals = { prompt: 0, completion: 0, requests: 0, billable: 0 };
  checklist = [];
  runEventLog = [];
  chrome.storage.session.remove(['bat_session', 'bat_goal', 'bat_ui', 'bat_usage', 'bat_debug', 'bat_checklist', 'bat_eventlog'])
    .catch((e) => debugEntry('clear_session_failed', { error: e?.message }));
  chrome.storage.local.remove(['bat_session', 'bat_goal', 'bat_ui', 'bat_usage', 'bat_debug', 'bat_checklist', 'bat_eventlog', 'bat_saved_at'])
    .catch((e) => debugEntry('clear_local_session_failed', { error: e?.message }));
  chatEl.innerHTML = '';
  updateDebugStatus();
  // Clear all collected data and workspace files. Await both and report the
  // truth: "Cleared" used to be shown unconditionally even when the store or
  // the folder wipe had failed, so a partial clear looked complete.
  Promise.allSettled([Store.clearAll(), Workspace.clearFiles()]).then((results) => {
    const storeOk = results[0].status === 'fulfilled';
    const filesOk = results[1].status === 'fulfilled';
    if (storeOk && filesOk) {
      addMessage('system', 'Cleared — chat, collected data, and files.');
    } else {
      if (results[0].status === 'rejected') {
        debugEntry('clear_all_failed', { error: results[0].reason?.message || String(results[0].reason) });
      }
      if (results[1].status === 'rejected') {
        debugEntry('clear_files_failed', { error: results[1].reason?.message || String(results[1].reason) });
      }
      addMessage('system', 'Chat cleared, but some data could not be removed — see Debug report for details.');
    }
  });
}
...
function renumberTreeRefs(tree, frameId, registry, numbering) {
  ...
  for (const line of lines) {
    // A tree line can carry MORE than one [ref_N] (some Storyline trees nest
    // multiple refs on one control line). The old code matched only the first
    // and left the rest frame-local, so a second ref on the same line kept its
    // per-frame number and could collide with a different frame's global ref of
    // the same number → clicks on the wrong element. Rewrite every occurrence.
    const m = line.match(/\[ref_(\d+)\]/g);
    if (!m) { out.push(line); continue; }
    let rewritten = line;
    for (const token of m) {
      const localRef = token;
      ...
      rewritten = rewritten.replace(token, '[' + globalRef + ']');
    }
    const labelMatch = line.match(/"((?:[^"\\]|\\.)*)"/);
    const firstLocal = m[0];
    const firstKey = frameId + ':' + firstLocal;
    registry[numbering.byKey.get(firstKey)] = {
      frameId,
      localRef: firstLocal,
...
async function tryDismissResumeDialog(tabId, resumeClickStreak = 0) {
  ...
  // Auto-dismiss CLICKS a button on the page. It is routed through
  // observePage → buildSnapshot → read_page, NOT through the guarded-tool
  // check that gates left_click — so a course page's Resume/Restart dialog was
  // being clicked even on hosts the user never approved for page actions. That
  // is an allowlist bypass for a page-changing action; honour the same
  // boundary as every other click before doing anything.
  const access = await checkTabAccess(tabId);
  if (!access.ok) {
    debugEntry('auto_dismiss_skipped_unapproved', { tabId, host: access.host, reason: access.reason });
...
function lookupRefInFrame(localRef) {
  try {
    ...
    if (!el) {
      // localRef is model-supplied; escape before interpolation into a CSS
      // selector so a hostile ref cannot throw or match a different element.
      const q = typeof CSS !== 'undefined' && CSS.escape ? CSS.escape(localRef) : String(localRef).replace(/["\\]/g, '\\$&');
      el = document.querySelector('[data-ext-ref="' + q + '"]');
    }
...
async function findCoordsByRegistryLabel(tabId, needle) {
  ...
  // A fuzzy relocation ends in a CLICK, so "best available match" is not a good
  // enough bar — rankByLabel's 0.2 floor is a search threshold, not an action
  // threshold. Below this score the deterministic scan further down (exact
  // label, then substring) still gets its chance; what is refused is acting on a
  // guess. 0.6 admits the real fuzzy case ("mens trucker jacket" ≈ "Levi's
  // Men's Trucker Jacket", 0.76) while refusing the confusable ones that made
  // this worth guarding: "Question 1" vs "Question 2" (0.33) and "Add to cart"
  // vs "Add to bag" (0.5).
  const MIN_CLICK_SIMILARITY = 0.6;
  const ranked = findBestRef(registry, needle);
  if (ranked && ranked.score < MIN_CLICK_SIMILARITY) {
    debugEntry('relocate_rejected_low_confidence', {
      needle: String(needle).slice(0, 80),
      bestLabel: (ranked.label || '').slice(0, 80),
...
// Assemble a page observation into ONE clean, structured report the model can
// reliably parse. The old output concatenated hints, title, a raw tree dump and
// a separate text block with no real structure, so the model had to re-derive
// "what kind of page is this and what can I act on" from a wall of indented
// lines. This uses explicit section headers and a short facts block instead.
function buildPageContext(goal, pageData, tabId = null, resumeClickStreak = 0) {
  ...
  // ── facts: a few high-signal lines, always in the same shape ──
  let ctx = 'PAGE REPORT\n';
  ctx += `- title: ${title}\n`;
  ctx += `- url: ${url}\n`;
  const modeBits = [];
  if (quiz.isQuiz) modeBits.push(quiz.multi ? 'quiz (multi-select)' : 'quiz (single)');
  if (quiz.isQuiz) modeBits.push(`${quiz.checked}/${quiz.total} answered`);
  ctx += `- mode: ${modeBits.length ? modeBits.join(', ') : 'normal page'}\n`;
  ctx += `- actions: ${checkboxRefs.length + (accessibilityTree.match(/\[ref_\d+\]/g) || []).length} interactive element(s)`;

  // ── phase hints (course-specific) ──
  const resumeHint = buildResumeDialogHint(pageData, tabId, resumeClickStreak);
  const quizHint = buildQuizPhaseHint(pageData);
  if (resumeHint || quizHint) {
    ctx += '\n';
    if (resumeHint) ctx += resumeHint.trimEnd() + '\n';
    if (quizHint) ctx += quizHint.trimEnd() + '\n';
  }
  ctx += '\n';

  // ── the tree, cleanly fenced so the model knows where it starts/ends ──
  if (accessibilityTree) {
    ctx += `--- ACCESSIBILITY TREE ---\n${extractFocusedTree(accessibilityTree, pageData)}`;
  } else {
    ctx += `--- ACCESSIBILITY TREE ---\n(empty — use page_text below)`;
  }

  // ── plain text, only when the tree alone is too weak to act on ──
  const pageText = enrichedText || text;
  if (pageText) {
    ctx += `\n\n--- PAGE TEXT ---\n${extractFocusedText(pageText, pageData)}`;
  }

  // ── checkbox refs the model can click directly ──
  ...
  if (checkboxSummary.length && !checkboxRefs.length) {
...
// ── Delegated file writes from the background runner ──────────────
// Chrome binds a folder grant to the document that requested it — this panel —
// so the service worker cannot write to a pinned folder at all. It therefore
// hands those appends here instead of failing them, which is what used to make
// a healthy folder look like it "kept disconnecting" during a background run.
// A separate listener because this one must return `true` to answer async.
chrome.runtime.onMessage.addListener((message, _sender, sendResponse) => {
  if (message?.type !== 'BAT_WS_APPEND') return false;
  (async () => {
    try {
      const result = await writerAppendRows(message.filename, message.rows, message.opts || {});
...
// Bring the file back in line with the store for any run whose projection fell
// behind while the panel was closed. The store is the authority and always has
// the rows; this is the projection catching up. It used to require the model to
// notice `pendingExport` and call export_rows — which it often did not, so the
// file silently lagged the store for the rest of the session.
async function reconcilePendingExports() {
  let runs;
  try { runs = await Store.listRuns(); } catch { return; }
  const stale = runs.filter(r => r.pendingExport && r.plan?.filename);
  if (!stale.length) return;

  // One run per FILE — several runs can target the same file, and writeAll
  // rewrites the whole thing from the store, so doing it once is both correct
  // and cheaper.
  const byFile = new Map();
  for (const r of stale) byFile.set(Workspace.safeName(r.plan.filename), r);

  for (const [collection, run] of byFile) {
    try {
      const recs = await Store.getRows(collection);
      if (!recs.length) continue;
      const sourceField = run.plan.source_field || 'source';
      const r = await writerWriteAll(run.plan.filename, recs.map(rec => Store.projectRow(rec, sourceField)), {
        columns: run.plan.columns,
        format: run.plan.format
      });
      addActivity(null, 'result', 'File brought up to date',
        `${r.name}: ${r.rowCount} unique rows written from the store (the background run could not write it while the panel was closed).`,
        'success', false, false);
    } catch (e) {
      // Still pending — say so plainly rather than leaving the divergence silent.
      addActivity(null, 'warn', 'File is behind the store',
        `${collection} could not be written: ${e.message}`, 'warn', false, false);
      continue;
    }
    for (const r of stale) {
      if (Workspace.safeName(r.plan.filename) !== collection) continue;
      r.pendingExport = false;
      r.writeError = '';
      await Store.saveRun(r).catch(() => {});
    }
  }
}
...
chrome.runtime.onMessage.addListener((message) => {
  ...
  if (ev === 'awaiting_human') {
    ...
    // The runner reuses the `run_done` event for its terminal-failure path with
    // `failed: true` — never report a crash as a success.
    if (message.failed) {
      addActivity(null, 'error', 'Background run FAILED', message.note || 'the run crashed and could not continue', 'error');
    } else {
      addActivity(null, 'done', 'Background run complete', `${message.counts?.pages ?? 0} pages · ${message.counts?.rows ?? 0} rows collected.`, 'success');
    }
    // The run is over; sync the file to the store now rather than leaving it to
    // whether the model remembers to call export_rows.
    reconcilePendingExports();
  } else if (ev === 'file_write_failed') {
    // Reaches the panel only for GENUINE failures now — a background run that
    // merely could not reach a pinned folder from the worker is deferred, not
    // failed, and never gets here.
    addActivity(null, 'warn', 'File not written — rows are safe in the store',
      `${message.note || 'unknown error'}\nThe store keeps every row; the file is a projection of it and will be rewritten on the next export.`, 'warn');
  } else if (ev === 'budget_stop') {
    addActivity(null, 'warn', 'Run paused — token budget reached', `${message.note || ''}\nSay "resume the run" to continue.`, 'warn');
...
if (settingsBtn && settingsPanel) {
  ...
  settingsBtn.addEventListener('click', () => {
    // Settings REPLACES the chat interface: a class on <body> swaps the whole
    // view, so the settings screen is not a strip squeezed next to the chat —
    // it IS the panel while open, and the same button returns to the chat.
    const open = !document.body.classList.contains('show-settings');
    document.body.classList.toggle('show-settings', open);
    settingsBtn.setAttribute('aria-expanded', String(open));
    // The settings screen is now the only place these are visible, so they have
    // to be current the moment it opens rather than only at startup. Each can
    // reject (IDB/worker failure) — swallow into the debug log rather than
    // leaving an unhandled rejection in an event handler.
    if (open) {
      updateWorkspaceStatus().catch((e) => debugEntry('settings_workspace_status_failed', { error: e?.message || String(e) }));
      updateDataStatus().catch((e) => debugEntry('settings_data_status_failed', { error: e?.message || String(e) }));
      updateDebugStatus();
    }
  });
  const settingsCloseBtn = document.getElementById('settingsCloseBtn');
  if (settingsCloseBtn) settingsCloseBtn.addEventListener('click', closeSettings);
  // Escape dismisses the TOPMOST layer first: if the stored-data panel is open
  // over the settings screen, close that (not the settings beneath it); only
  // when it's clear does Escape return to the chat.
  document.addEventListener('keydown', (e) => {
    if (e.key !== 'Escape') return;
    if (dataPanel && !dataPanel.hidden) {
      dataPanel.hidden = true;
      updateDataStatus();
      return;
    }
    if (document.body.classList.contains('show-settings')) closeSettings();
  });
}
...
if (workspaceReconnectBtn) {
  workspaceReconnectBtn.addEventListener('click', async () => {
    ...
    if (res.ok) {
      ...
      // Anything that piled up while it was disconnected is written now, so the
      // user does not have to know that export_rows is the follow-up step.
      reconcilePendingExports();
    } else {
      ...
      actions: [{
        ...
        onClick: async () => {
          ...
          try {
            ...
            debugEntry('data_delete_collection_failed', { collection: c.collection, error: e?.message || String(e) });
            addActivity(null, 'warn', 'Could not delete collection', `${c.collection}: ${e?.message || e}`, 'warn');
          }
          renderDataPanel();
        }
      }]
      ...
      actions: [{
        ...
        onClick: async () => {
          ...
          try {
            ...
            debugEntry('data_delete_extractor_failed', { pattern: x.pattern, error: e?.message || String(e) });
            addActivity(null, 'warn', 'Could not delete extractor', `${x.pattern}: ${e?.message || e}`, 'warn');
          }
          renderDataPanel();
        }
      }]
      ...
      actions.push({
        ...
        onClick: async () => {
          ...
          try {
            ...
            debugEntry('data_delete_run_failed', { runId: r.id, error: e?.message || String(e) });
            addActivity(null, 'warn', 'Could not delete run', `${r.id}: ${e?.message || e}`, 'warn');
          }
          renderDataPanel();
        }
      });
...
(async function init() {
  try {
    ...
    if (ws.needsReconnect) {
      addMessage('system',
        `Folder "${ws.location}" needs reconnecting (Chrome drops the permission on restart). `
        + 'Settings ⚙ → Reconnect folder. Nothing is lost meanwhile.',
        { persist: false });
    }
    updateWorkspaceStatus();
    updateDebugStatus();
    ...
    if (!getActiveKey()) {
      // No key → swap straight to the settings screen so the user can paste one.
    ...
    // The panel is the only context that can write a pinned folder, so opening
    // it is exactly when a file left behind by a background run can catch up.
    reconcilePendingExports();
  } catch (e) {
