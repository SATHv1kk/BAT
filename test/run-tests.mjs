...
if (!ONLY_LIVE) {
  ...
  t('missing latest (deleted run) is a stop', rc.stop === true && rc.externalStatus === 'deleted');
  t('deleted run is not resurrected', rc.record === loopRun);
  ...
  for (const mod of ['redaction.test.mjs', 'allowlist.test.mjs', 'extractor-screen.test.mjs',
    'extractor-ast-screen.test.mjs', 'regex-guard.test.mjs', 'embedded-storage.test.mjs',
    'workspace-integration.test.mjs', 'output-writer.test.mjs', 'parse.test.mjs',
    'site-verification.test.mjs', 'deepseek-request.test.mjs', 'locator.test.mjs',
    'jsonld-parser.test.mjs']) {
