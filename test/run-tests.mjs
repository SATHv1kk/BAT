...
console.log('Phase 5 — ATS adapters (live)');
...
t('risky sites flagged by name', ['linkedin.com','ie.indeed.com','glassdoor.ie'].every(s => SITE_CONFIGS.find(c => c.site === s)?.account_risk === true));
...
const { STOP_PATTERNS } = await import(B + 'lib/plan.js');
{
  ...
  t('week rule stops old postings', mustStop.every(s => wk.test(s)));
  t('week rule keeps fresh postings', mustKeep.every(s => !wk.test(s)));
