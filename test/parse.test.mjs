...
export default function run(t) {
  ...
  t('comma before brace inside a string is not stripped', parseAgentResponse('{"action":"wait","text":"keep ,} here"}').text === 'keep ,} here');
  t('comma before bracket inside a string is not stripped', parseAgentResponse('{"action":"find","value":"a, ] b"}').value === 'a, ] b');
  t('trailing comma in nested object stripped', parseAgentResponse('{"action":"batch","actions":[{"action":"wait",}],}').action === 'batch');
  ...
  t('type with explicit empty value is a clear', norm('{"action":"type","ref":"ref_1","value":""}').action === 'type');
  t('type with explicit empty value keeps empty', norm('{"action":"type","ref":"ref_1","value":""}').value === '');
  t('type with numeric zero is kept', norm('{"action":"type","ref":"ref_1","value":0}').action === 'type' && norm('{"action":"type","ref":"ref_1","value":0}').value === '0');
  t('form_input with zero value is kept', norm('{"action":"form_input","ref":"ref_1","value":0}').value === 0);
